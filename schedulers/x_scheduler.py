#!/usr/bin/env python3
"""
Active Scheduler - Periodically runs the X agent in 'active' mode to
simulate natural social media engagement.
"""
...
async def active_loop(theme: str, interval_min: int, interval_max: int, duration_min: int, duration_max: int):
	"""Run the agent in active mode on a schedule."""
	# Import here to avoid circular imports / heavy loading at module level
	from agents.x import run_agent

	log.info(f'🕐 Active Scheduler Started')
	...
	log.info(f'   Log file: {LOGS_DIR / "scheduler.log"}')
	...
	while True:
		try:
			...
			log.info(f'🔄 [{now}] Starting active session #{run_count} (duration: {duration} min)...')
			...
			log.error(f'❌ Error in active session: {e}')
			log.info('   Waiting 5 minutes before retry...')
			await asyncio.sleep(300)


async def main():
	parser = argparse.ArgumentParser(description='X.com Active Scheduler')
	parser.add_argument('--theme', type=str, default='tech and AI',
